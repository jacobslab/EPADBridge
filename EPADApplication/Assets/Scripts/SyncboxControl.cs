...
public class SyncboxControl : MonoBehaviour
{
	...
	EPADApplication exp { get { return EPADApplication.Instance; } }
    ...
    {
		UnityEngine.Debug.Log("Syncing immediately");
		ToggleLEDOn();
		yield return StartCoroutine(WaitForShortTime(syncPulseDuration));
		ToggleLEDOff();
		yield return null;
	...
	void LogSYNCOn(long time)
	{
		...
		{
			exp.timeSyncLog.LogSyncOn(true, time);
		}
	}
	...
	void LogSYNCOff(long time)
	{
		if (Configuration.isLogging)
		{
			exp.timeSyncLog.LogSyncOn(false, time);
		}
	}
